...
## 🌍 The GeoLogistics & Comm Factories

To maintain spatial testing boundaries, we implement `GeoLogisticsFactory` natively parsing Google APIs or retreating to Sandbox Bounds when keys are omitted. Likewise, the `CommFactory` restricts all outputs from rogue UI loops via hard-coded strings. Ensure these remain actively decoupled.

Note: `docker-compose.yml` now includes `db` and optional `redis` services; see the updated config for corresponding required environment variables.
