...
The audit system exposes three endpoints. The core implementation lives in `app/skills/audit_system.py` providing a secure `AuditDispatcher`, `CVEMonitor`, and `DependencyScanner` conforming to the architectural layout of `08_AGNOSTIC_FACTORIES.md` and hooked natively through `app/main.py`.
