...
*(Note: Scheduled audit deep-dive lives in `09_AUDIT_AND_MAINTENANCE.md`)*
...
**✅ Long-Term Health**
- Scheduled audits catch dependency rot, API drift, and stale recommendations
...
**The Scheduled Audit:**
On a configurable schedule (recommended: every 6 months), the system runs an automated scan and produces a report. The human reviews it and approves each change. Nothing is auto-applied.
...
1. **Session Start:** Type `/status` — AI reads `.build-context.md`, summarizes state, lists active bugs and available skills.
...
3. **Session End:** Tell AI: "Update `.build-context.md` with what we accomplished." AI updates Current State, Recent Changes, Skills Registry, and Architectural Decisions.
